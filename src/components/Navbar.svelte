...
>
  ...
  <div class="flex items-center gap-0 sm:gap-2">
    <div class="max-sm:hidden w-4 h-4">
      <!-- Future logo placeholder -->
    </div>
    <h1 class="font-bold text-base md:text-lg tracking-tight"><a href="/">ToggleMind</a></h1>
  </div>
  ...
  <nav class="flex justify-end items-center gap-2 sm:gap-4 lg:gap-8">
    ...
    <div class="flex gap-2 sm:gap-4 font-bold">
      <button
        class="py-2 w-16 md:w-20 bg-sky-500 text-white rounded-xl text-sm md:text-base"
        >Sign up</button
      >
      <button
        class="py-2 w-16 md:w-20 bg-slate-200 text-black rounded-xl text-sm md:text-base"
        >Log in</button
      >
    </div>
  </nav>
